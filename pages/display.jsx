...
export default function Display() {
  ...
  async function pollBillboardData() {
    try {
      const { data, error } = await supabase.from('ad_bookings').select().order('createdAt', { ascending: false }).select('imgUrl').eq('billboardId', parseInt(router.query.billboardId))
      if(data === null)
