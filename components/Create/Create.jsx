...
export default function CreateBillboardFormContainer({session}) {
  ...
  return(
    <>
      <ImageUpload uid={user?.id} onUpload={(url) => { setImageUrl(url) }}/>
      <div>
        <label>Title</label>
      ...
      <LocationPicker updateLat={setLat} updateLng={setLng}/>
      <div>
        <label>Rate</label>
        <input id="rate" onChange={(e) => setRate(e.target.value)}/>
      </div>
      <Link href="/">
        <button className="button primary block" onClick={createBillBoard} disabled={loading}>
